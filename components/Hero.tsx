  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      particles.forEach((p) => {
        ...
        ctx.fillStyle = `rgba(200, 228, 245, ${p.opacity})`
        ctx.fill()
      })
      ...
      particles.forEach((a, i) => {
        particles.slice(i + 1).forEach((b) => {
          ...
          if (dist < 100) {
            ...
            ctx.moveTo(a.x, a.y)
            ctx.lineTo(b.x, b.y)
            ctx.strokeStyle = `rgba(200, 228, 245, ${0.15 * (1 - dist / 100)})`
            ctx.lineWidth = 0.5
            ctx.stroke()
          }
        })
        ...
        <div className="max-w-4xl">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 animate-fade-up" style={{ animationDelay: '0.45s' }}>
            {stats.map((stat) => (
              <div
                key={stat.label}
                className="glass-card rounded-2xl px-5 py-4 bg-white/10 border border-white/20 backdrop-blur-sm"
              >
